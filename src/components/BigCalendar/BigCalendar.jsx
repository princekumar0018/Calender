...
const BigCalendar = (props) => {
  ...
  function renderEvents(day, monthIndex) {
    ...
    events
      .filter((e) => filterEvents.includes(e.status))
      .forEach((event) => {
          ...
          <div key={eventDate}>
            {eventGroupByDate[eventDate].slice(0, 4).map((evt) => (
              <div
                key={evt._id}
                onClick={(e) =>
                  withPreventDefault(e, handleClickOnEventName(evt, monthIndex))
                }
                className="event-name"
                style={{
                  background:
                    colors[evt.eventColor] || statusColors[evt.status],
                }}
              >
                {evt.title || "Untitled"}
              </div>
            ...
            {more > 0 && (
              <div className="relative">
                <div
                  onClick={(e) => handleShowAllEvent(e, eventDate)}
                  className="event-name see-more-btn w-max"
                >
                  {" "}
                  {more} more...
                </div>
                ...
                {isShowAllEventDate && (
                  <div>
                    <Popup
                      className="popup all-event-popup-modal py-2 px-1"
                      onClose={(e) => handleShowAllEvent(e, eventDate)}
                      isWithBackdrop={true}
                      isOpen={
                        !!isShowAllEventDate && isShowAllEventDate === eventDate
                      }
                    >
                      <div
                        onClick={(e) => handleShowAllEvent(e, eventDate)}
                      ...
                      <div onClick={(e) => withStopPropagation(e, () => {})}>
                        <div>
                          <div className="ml-2 m-auto text-center text-gray-500">
                            ...
                            <h4
                              onClick={(e) =>
                                withStopPropagation(e, handleClickOnDate(day))
                              }
                              className="btn-circle m-auto flex items-center justify-center w-10 h-10 text-center text-xl text-gray-700 "
                            >
                              {day.date()}
                            </h4>
                          </div>
                        ...
                        {eventGroupByDate[eventDate].map((eachEvt, i) => (
                          <li
                            onClick={(e) =>
                              withStopPropagation(
                                e,
                                handleOpenEventDetailRoute(eachEvt._id)
                              )
                            }
                            key={i}
        ...
        <div>
          <div className="grid grid-cols-7 day-row">
            {weeks.map((week, weekIndex) => (
              <div
                key={weekIndex}
                style={{ background: weekDay[weekIndex] }}
                className="big-date"
              >
          ...
          <div className="grid grid-cols-7 grid-rows-6">
            {daysMatrix.map((row) =>
              row.map((day, dayIndex) => (
                <div
                  style={{ background: weekDay[dayIndex] }}
                  key={day.date()}
