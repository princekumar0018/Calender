...
const AddEventModal = () => {
  ...
  useEffect(() => {
    if (newEventData.isEventCreateInitialize) {
      ...
      setEvents((prev) => {
        ...
        let updateEventIndex = updateEvents.findIndex(
          (evt) => evt.isEventCreateInitialize
        );
    ...
    newEventData.isEventCreateInitialize,
    newEventData.title,
    newEventData.eventColor,
  ]);
  ...
  async function handleAddEvent(cb) {
    ...
    try {
      const invitationUsers =
        newEventData.invitations.map((user) => user._id) || [];
      ...
      if (newEventData.updateEventId) {
        ...
        axios
          .put("/api/calendar/update/" + newEventData.updateEventId, payload, {
            headers: {
          ...
          .then(({ data, status }) => {
            ...
            setEvents((prev) => {
              let updateEvents = [...prev];
              let updateEventIndex = updateEvents.findIndex(
                (evt) => evt._id === newEventData.updateEventId
              );
              if (updateEventIndex !== -1) {
                updateEvents[updateEventIndex] = {
                  ...updateEvents[updateEventIndex],
        ...
        axios
          .post("/api/calendar/create", payload)
          .then(({ data, status }) => {
            setCloseNewEventModal();
  ...
  return (
    <div>
      <MultiStepModal
        isOpen={newEventData.isOpen}
        onClose={handleClose}
        className="add-event-multi-modal"
      >
        <CSSTransition
          unmountOnExit={true}
          in={modalId === 1}
